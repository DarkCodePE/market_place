...
import com.example.multisecurityspring.domain.entity.TokenStatus;
import com.example.multisecurityspring.domain.entity.User;
import com.example.multisecurityspring.infrastructure.exception.InvalidTokenRequestException;
import com.example.multisecurityspring.infrastructure.repository.EmailVerificationTokenRepository;
...
public class EmailVerificationTokenService {
    ...
    public void verifyExpiration(EmailVerificationToken token) {
        if (token.getExpiryDate().compareTo(Instant.now()) < 0) {
            throw new InvalidTokenRequestException("api.token.fail", "Email Verification Token", token.getToken(), "Expired token. Please issue a new request");
        }
    }
}
